...
@section('title', 'تحرير الشهادة - لوحة التحكم')
@section('page-title', 'تحرير الشهادة')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
    <div class="flex items-center">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">تحرير الشهادة</h1>
            <p class="text-gray-600 mt-1">تحرير شهادة: {{ $testimonial->name }}</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    <form method="POST" action="{{ route('dashboard.testimonials.update', $testimonial->id) }}" enctype="multipart/form-data" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea name="opinion" 
                      ...
                      required>{{ old('opinion', $testimonial->opinion) }}</textarea>
            @error('opinion')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                {{ $testimonial->image ? 'تغيير الصورة' : 'إضافة صورة' }}
            </label>
            
            <!-- New Image Preview -->
            <div id="new-image-preview" class="mb-4 hidden">
                <div class="relative inline-block">
                    <img id="new-preview-img" src="" alt="معاينة الصورة الجديدة" class="w-32 h-32 object-cover rounded-lg border border-gray-200">
                    <button type="button" id="remove-new-image" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 transition-colors" title="إزالة الصورة">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const removeImageBtn = document.getElementById('remove-new-image');
