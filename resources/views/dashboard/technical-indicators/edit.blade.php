...
@section('title', 'تعديل المؤشر الفني - لوحة التحكم')
@section('page-title', 'تعديل المؤشر الفني')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">تعديل المؤشر الفني</h2>
                <p class="text-gray-600">تعديل تفاصيل المؤشر الفني والفيديوهات</p>
            </div>
    ...
    <!-- Form -->
    <form action="{{ route('dashboard.technical-indicators.update', $technicalIndicator->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" 
                       id="title" 
                       name="title" 
                       value="{{ old('title', $technicalIndicator->title) }}"
            ...
            <div class="mb-6">
                ...
                <textarea id="description" 
                          ...
                          placeholder="أدخل وصف المؤشر الفني">{{ old('description', $technicalIndicator->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            @if($technicalIndicator->videos && count($technicalIndicator->videos) > 0)
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">الفيديوهات الحالية</label>
                    <div class="space-y-2">
                        @foreach($technicalIndicator->videos as $index => $video)
                            <div class="flex items-center space-x-reverse space-x-2 p-2 bg-gray-50 rounded-lg">
                                <span class="text-sm text-gray-600">{{ $index + 1 }}.</span>
                                <span class="flex-1 text-sm text-gray-800 truncate">{{ $video }}</span>
                            </div>
                        @endforeach
                    </div>
                    <p class="text-xs text-gray-500 mt-2">أدخل فيديوهات جديدة لاستبدالها</p>
                </div>
            @endif
            ...
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    {{ ($technicalIndicator->videos && count($technicalIndicator->videos) > 0) ? 'استبدال فيديوهات يوتيوب' : 'فيديوهات يوتيوب' }}
                </label>
            ...
            <div class="mb-6">
                ...
                <input type="datetime-local" 
                       id="published_at" 
                       name="published_at" 
                       value="{{ old('published_at', $technicalIndicator->published_at ? $technicalIndicator->published_at->format('Y-m-d\TH:i') : '') }}"
            ...
            <div class="mb-6">
                <div class="flex items-center">
                    <input type="checkbox" 
                           ...
                           name="is_active" 
                           value="1"
                           {{ old('is_active', $technicalIndicator->is_active) ? 'checked' : '' }}
